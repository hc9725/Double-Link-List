...
void quicksort(Dlinklist leftlist, Dlinklist rightlist , int left,int right, int cmpall(const void *a,const void *b))
{
	...
	while(left < right)
	{
		while(curleft < curright && (cmpall(baselist->data,currightlist->data) > 0))
		{
			printf("curleft is %d  curright is %d     ",curleft,curright);
		...
		while(curleft < curright && (cmpall(baselist->data,curleftlist->data) < 0))
		{
			printf("curleft is %d  curright is %d     ",curleft,curright);
			printf("the curleftnode is  %c   the curright node is %c  \n",*(char *)curleftlist->data,*(char *)currightlist->data);
		...
		q->prior->next = q;
		q->prior = q;
		curleftlist = p->next;//back where it was
		currightlist = q->prior;
	...
	baselist->next = curleftlist->next;
	baselist->prior = curleftlist;
	curleftlist->next->prior = baselist;
	curleftlist->next = baselist;
	quicksort(leftlist->next,curleftlist,left,curleft,cmpall);//recurse
	quicksort(rightlist->prior,currightlist,curleft,right,cmpall); 
